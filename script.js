...
document.querySelectorAll('.action-btn.save').forEach((button) => {
  button.addEventListener('click', (e) => {
    const id = e.target.parentElement.querySelector('textarea').id;
    const value = document.getElementById(id).value;
    localStorage.setItem(`livecode-${id}`, JSON.stringify(value));
    swal('Saved!', 'Your code has been saved', 'success');
  });
});

document.querySelectorAll('.action-btn.lock').forEach((button) => {
  button.addEventListener('click', (e) => {
    const id = e.target.parentElement.querySelector('textarea').id;
    const textarea = document.getElementById(id);
    textarea.readOnly = !textarea.readOnly;
    e.target.textContent = textarea.readOnly ? 'Unlock' : 'Lock';
  });
});
